...
<head>
	...
	<title> X-Trade - [<%=session.getAttribute("username")%>] Profile </title>
...
<%
	...
	ResultSet Rs = stm.executeQuery("select * from user.users where id = '"+ uid + "'");
	Rs.next();
		...
		<tr>
			<td width="50%" style="border:1px solid black;">
				<div style="margin:0 auto; text-align:center; padding:30px;">
					<div >
						<img style="width:400px; height: 400px;" src="./ImageDisplayServlet?uid=<%Rs.getString("id");%>"/> <br/>
					</div>
				</div>
				<p style="font-size: 40px; font-weight:bold; text-align: center;"><%=Rs.getString("username")%></p>
			</td>
			...
			<td width="50%" style="padding-left: 5%; align-content:center;" >
				<table style="padding:15px 10px 20px 10px;">
					<tr>
						<td>Name :</td>
					...
					<tr>
						<td>Location :</td>
						<td style="text-align:center;" ><%=Rs.getString("location")%></td>
					</tr>
					<tr>
						<td>Mobile Number:</td>
						<td style="text-align:center;"><%=Rs.getString("mobile")%></td>
					</tr>



				</table>
				<button class="button" style="width: 45%; margin-left: 100px;" > <p><a style="font-size: 25px;" href="NewAccountSetting.jsp"><b>Edit Profile</b></a></p></button>
			</td>
		</tr>
...
<%
	...
	ResultSet rs = Stm.executeQuery("SELECT Distinct * FROM user.product\n" +
			"where (product_category = 'Clothes & Accessories'or product_category ='Books & Stationary') and user_id = '" + uid + "'\n" +
			"ORDER BY RAND()\n" +
			"LIMIT 1;");
...
<!-- Trending Products -->
<div class="row">
	<h2 class="w3-wide w3-center" style="margin-top:100px; font-size: 45px; font-weight: bold; font-family:Times New Roman;">Product Sell by <%=Rs.getString("name")%></h2>
	<div class="column">
		<div class="card">
			<img src="./ProductDisplayServlet?product_id=<%=rs.getString("product_id")%>"style="width:200px; height:200px;"/>
			<div class="container">
				...
				<p style="font-size: 20px">Price : <%=rs.getString("price")%></p>
	...
	<%
		...
		Statement STm = Connection.createStatement();
		ResultSet RS = STm.executeQuery("SELECT Distinct * FROM user.product\n" +
				"where (product_category = 'Household Appliance' or product_category ='Others')and user_id = '" + uid + "'\n" +
				"ORDER BY RAND()\n" +
				"LIMIT 1;");
	...
	<%
		...
		ResultSet RES = STM.executeQuery(" SELECT Distinct * FROM user.product\n" +
				"where (product_category = 'Food & Beverage') and user_id = '" + uid + "'\n" +
				"ORDER BY RAND()\n" +
				"LIMIT 1;");
	...
	<div class="column">
		<div class="card">
			<img src="./ProductDisplayServlet?product_id=<%=RES.getString("product_id")%>"style="width:200px; height:200px;"/>
			<div class="container">
				...
				<p style="font-size: 20px">Price : <%=RES.getString("price")%></p>
	...
	<a href="SellerAllProduct.jsp?uid=<%=RES.getString("user_id")%>">
		<button class="button" style="width: 200px;margin-top: 20px; margin-left: 43.8%; " >View More Product</button>
	</a>
</div>
